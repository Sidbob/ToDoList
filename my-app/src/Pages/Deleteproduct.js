...
const DeleteProduct = () => {
    ...
    const deleteProduct = async (id) => {
        console.log(id)
        const data = await axios.delete(`http://localhost:9027/user/delete-product/${id}`, {
    ...
    return (
        <div>
            ...
            <input type="text" placeholder='Enter Product' value={name} />
            <input type="text" placeholder='Enter Product Id' value={price} />

            {listproduct.map((list, index) => {
                return (
                    <div className="content-box" key={index}>
                        <h2>Product Name:{list.name}</h2>
                        ...
                        <button className='updatebtn'><Link style={{textDecoration:"none", color:"white"}} to={"/update-old-category/"+list._id}>Update</Link></button>
                        </div>
                    </div>
                )
            })}
